...
# Αρχική τιμή (θα ενημερωθεί μετά τον πρώτο έλεγχο)
previous_price = 0.0
...
def get_current_price():
    ...
    try:
        ...
        # Εντοπισμός του ChromeDriver στο σύστημα του GitHub Actions
        service = Service('/usr/bin/chromedriver') # Ο ChromeDriver βρίσκεται σε αυτή τη διαδρομή στα συστήματα του GitHub Actions
        driver = webdriver.Chrome(service=service, options=chrome_options)

        driver.get(URL)
        time.sleep(5)  # Αναμονή για να φορτώσει η σελίδα

        # Εντοπισμός του στοιχείου με την τιμή.
        price_element = driver.find_element(By.XPATH, '//*[@id="productPage"]/div[2]/div/div[2]/div[1]/div[1]/span/span[2]')
        price_text = price_element.text
        
        # Καθαρισμός και μετατροπή της τιμής
...
def send_notification(message):
    ...
    try:
        pushover_api = Push(PUSHOVER_USER_KEY, PUSHOVER_API_TOKEN)
        pushover_api.send_message(message, title="Ειδοποίηση Τιμής Χρυσής Λίρας")
...
if __name__ == "__main__":
    ...
    # Η λογική ελέγχου επαναλαμβάνεται μία φορά, καθώς το GitHub Actions θα τρέχει το script σε κάθε προγραμματισμένη εκτέλεση
    current_price = get_current_price()
    
    if current_price is not None:
        # Σε κάθε εκτέλεση, στέλνουμε την τρέχουσα τιμή
